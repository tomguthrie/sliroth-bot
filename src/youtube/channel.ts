...
export function parseYouTubeChannelId(input: string): string | undefined {
  ...
  const channelId =
    pathname === undefined ? undefined : /^\/channel\/([^/]+)(?:\/|$)/.exec(pathname)?.[1];

  return channelId !== undefined && isYouTubeChannelId(channelId) ? channelId : undefined;
}
...
export function parseYouTubeChannelHandle(input: string): string | undefined {
  ...
  return handle !== undefined && isYouTubeChannelHandle(handle) ? handle : undefined;
}
...
export async function fetchYouTubeChannelTitle(channelId: string): Promise<string> {
  ...
  return YouTubeChannelFeed.parse(parse(await response.text(), XML_PARSE_OPTIONS)).feed.title;
}

/** Resolves a YouTube channel ID, handle, or channel URL without the Data API. */
export async function resolveYouTubeChannel(input: string): Promise<YouTubeChannel | undefined> {
...
async function resolveYouTubeHandle(handle: string): Promise<string | undefined> {
  const url = new URL(`@${encodeURIComponent(handle)}`, 'https://www.youtube.com/');
  ...
  const match = /"externalId":"(UC[A-Za-z0-9_-]{22})"/.exec(await response.text());
...
function parseYouTubeUrlPathname(input: string): string | undefined {
  try {
    ...
    return url.protocol === 'https:' && /^(?:www\.|m\.)?youtube\.com$/i.test(url.hostname)
      ? url.pathname
      : undefined;
  } catch {
    return undefined;
