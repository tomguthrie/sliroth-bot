...
function parseYouTubeCommand(
  value: unknown,
): { name: string; options: unknown[] } | undefined {
  if (!isInteractionRecord(value) || value.name !== youtubeCommand.name) {
    return undefined;
  }
...
function isSupportedNotificationChannel(value: unknown): boolean {
  if (!isInteractionRecord(value)) {
    return false;
  }
  return (
    value.type === ChannelTypes.GUILD_TEXT ||
    value.type === ChannelTypes.GUILD_ANNOUNCEMENT
  );
}
