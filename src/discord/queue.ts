...
export type DiscordMessageDelivery = DiscordCreateMessageDelivery | DiscordEditMessageDelivery;

interface DiscordMessageQueue {
  sendBatch(messages: Iterable<MessageSendRequest<DiscordMessageDelivery>>): Promise<unknown>;
}
...
export function createDiscordMessageProcessor(
  ...
  return async (delivery, env, context) => {
    try {
      ...
      if (error instanceof DiscordApiError && error.status !== 429 && error.status < 500) {
...
function getRetryDelaySeconds(error: unknown): number | undefined {
  ...
  return Math.min(MAX_QUEUE_RETRY_DELAY_SECONDS, Math.max(1, Math.ceil(error.retryAfterSeconds)));
}
...
function createRetryDisposition(delaySeconds: number | undefined): QueueMessageDisposition {
  return delaySeconds === undefined ? { action: 'retry' } : { action: 'retry', delaySeconds };
}
